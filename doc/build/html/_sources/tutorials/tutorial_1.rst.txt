...
.. note::
	...
	Trekker does not require input nifti files to be in the same image space, i.e.: spatial dimensions of the FOD image can be 100x100x100 and the seed image can be 128x128x60. As long as images are in the same physical space, Trekker will handle nifti files with different headers.
...
- **an FOD image**: specified with option **-fod**, that consists of spherical harmonics coefficients (even orders only),
...
- All streamlines are saved in the physical coordinate space. 

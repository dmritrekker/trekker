...
inline void PTF::prepCandStepPropagator_C1() {
    PP[0] = TRACKER::stepSize;
    PP[4] = k1_cand*TRACKER::stepSize;
    PP[5] = k2_cand*TRACKER::stepSize;
    PP[1] = PP[4]*TRACKER::stepSize/2;
    PP[2] = PP[5]*TRACKER::stepSize/2;
    PP[8] = 1-k2_cand*PP[2];
    PP[3] = PP[8]-k1_cand*PP[1];
...
inline void PTF::prepCandStepPropagator_C2() {
    ...
    PP[1] = k1*tto2;
    PP[2] = k2*tto2;
    PP[3] = 1 + (-k1*k1 - k2*k2)*tto2;
    PP[4] = -t*((k1-k1_cand)*t/twos - k1);
    PP[5] = -t*((k2-k2_cand)*t/twos - k2);
    PP[6] = -PP[5];
    PP[7] = -k1*k2*tto2;
    PP[8] = 1-k2*k2*tto2;
}
...
inline void PTF::prepCandStepPropagator_C3(float r,float xc,float yc,float alpha,float theta) {
    ...
    PP[0] = -((2*cosa*r*tt*xc + 2*sina*r*tt*yc - 6 + (rr + xcxc + ycyc)*tt)*t)/6;
    PP[1] = -((r*sina*theta*t - (3*cosa*r*s)/2 - (3*xc*s)/2)*tt)/(3*s);
    PP[2] =  ((r*cosa*theta*t + (3*s*(r*sina + yc))/2)*tt)/(3*s);
    PP[3] =  (-2*r*tt*(t*theta*yc + xc*s)*cosa + 2*r*tt*(t*theta*xc - yc*s)*sina - s*(-2 + (rr + xcxc + ycyc)*tt))/(2*s);
    PP[4] =  1/(12*sss)*(t*(-7*rr*(t*theta*yc + (4*xc*s)/7)*tt*ss*cosacosa - 2*(-(7*r*tt*(t*theta*xc - (4*yc*s)/7)*s*sina)/2 + (-6 + (rr + 3*xcxc + ycyc)*tt)*ss + (5*ttt*xc*yc*theta*s)/2 + 4*tt*thetatheta)*r*s*cosa + (-4*t*xc*yc*sss + (-12 + (rr + 6*xcxc + ycyc)*tt)*theta*ss + 4*tt*thetacube)*r*t*sina + 2*ss*(-s*xc*(-6 + (rr + xcxc + ycyc)*tt) + rr*ttt*theta*yc)));
    PP[5] =  -((r*(2*r*tt*xc*ss*cosa + (-6 + (rr + xcxc + 3*ycyc)*tt)*ss + 4*tt*thetatheta)*sina - 2*s*(rr*tt*yc*s*cosacosa - r*t*(t*xc*yc*s - 3*theta)*cosa - 3*(-2 + (rr + xcxc/3 + ycyc/3)*tt)*s*yc/2))*t)/(6*ss);
    PP[6] = -PP[5];
    PP[7] =  -tt*(6*cosacosa*rr*t*theta + 3*cosa*sina*rr*s + 4*cosa*r*t*theta*xc - 2*sina*r*t*theta*yc + 3*cosa*r*yc*s + 3*sina*r*xc*s - 2*rr*t*theta + 3*xc*yc*s)/(6*s);
    PP[8] =   (cosacosa*rr*tt*s - 2*r*ttt*theta*(r*sina + yc)*cosa - (2*sina*r*tt*yc - 2 + (rr + ycyc)*tt)*s)/(2*s);
...
inline void PTF::walkCandStep(float *init_p, float **init_F) {
    ...
    for (int i=0; i<3; i++) {
        init_p[i]     = p[i] + PP[0]*F[0][i] +  PP[1]*F[1][i]  +  PP[2]*F[2][i];
        init_F[0][i]  =        PP[3]*F[0][i] +  PP[4]*F[1][i]  +  PP[5]*F[2][i];
        init_F[2][i]  =        PP[6]*F[0][i] +  PP[7]*F[1][i]  +  PP[8]*F[2][i];
    }

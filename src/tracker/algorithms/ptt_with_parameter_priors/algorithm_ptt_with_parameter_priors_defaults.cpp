...
void TrackWith_PTT_with_parameter_priors::setDefaultParametersWhenNecessary() {

	// Handle stepSize
	if (TRACKER::stepSize<=0.0) {
		TRACKER::stepSize = TRACKER::smallestPixDim*DEFAULT_PTT_WITH_PARAMETER_PRIORS_STEPSIZE_IN_PIXELDIM;
	...
	if (TRACKER::minRadiusOfCurvature<=0.0) {
		TRACKER::minRadiusOfCurvature = TRACKER::smallestPixDim*DEFAULT_PTT_WITH_PARAMETER_PRIORS_MINRADIUSOFCURVATURE_IN_PIXELDIM;
	...
	if (TRACKER::probeCount==NOTSET && TRACKER::probeRadius==NOTSET) {
		TRACKER::probeCount = DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROBECOUNT;
		TRACKER::probeRadius= DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROBERADIUS_IN_PIXELDIM*TRACKER::smallestPixDim;
	} else if (TRACKER::probeCount==NOTSET && TRACKER::probeRadius!=NOTSET) {
		TRACKER::probeCount = DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROBECOUNT_WHEN_THEREIS_PROBERADIUS;
	} else if (TRACKER::probeCount!=NOTSET && TRACKER::probeRadius==NOTSET) {
		TRACKER::probeRadius= DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROBERADIUS_IN_PIXELDIM*TRACKER::smallestPixDim;
	}
	...
	if (TRACKER::probeLength<=0.0) {
		TRACKER::probeLength = TRACKER::smallestPixDim*DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROBELENGTH_IN_PIXELDIM;
	...
	if ((TRACKER::probeQuality<=1.0) || (TRACKER::probeQuality>100)) {
		TRACKER::probeQuality = DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROBEQUALITY;
	...
	if (TRACKER::dataSupportExponent<0.0) TRACKER::dataSupportExponent = DEFAULT_PTT_WITH_PARAMETER_PRIORS_DATASUPPORTEXPONENT;
	
	// Handle minFODamp
	if (TRACKER::minFODamp<0.0) TRACKER::minFODamp = DEFAULT_PTT_WITH_PARAMETER_PRIORS_MINFODAMP;
	...
	if (TRACKER::maxEstInterval<=0.0) {
		TRACKER::maxEstInterval = DEFAULT_PTT_WITH_PARAMETER_PRIORS_MAXESTINTERVAL;
	...
	if (TRACKER::writeInterval<=0.0) {
		TRACKER::writeStepSize = TRACKER::smallestPixDim*DEFAULT_PTT_WITH_PARAMETER_PRIORS_WRITEINTERVAL_IN_PIXELDIM;
	...
	if (minLength<=0.0) minLength = DEFAULT_PTT_WITH_PARAMETER_PRIORS_MINLENGTH; // so comparisons are faster

	// Handle maxLength
	if ((TRACKER::maxLength<=minLength) || (TRACKER::maxLength>DEFAULT_PTT_WITH_PARAMETER_PRIORS_MAXLENGTH)){
		TRACKER::maxLength = DEFAULT_PTT_WITH_PARAMETER_PRIORS_MAXLENGTH;
	...
	if (TRACKER::triesPerRejectionSampling==NOTSET) {
		TRACKER::triesPerRejectionSampling = DEFAULT_PTT_WITH_PARAMETER_PRIORS_TRIESPERREJECTIONSAMPLING;
...
void TrackWith_PTT_with_parameter_priors::print() {
    
    if (GENERAL::usingAPI==false) {
        
        std::cout << "algorithm            : ptt_with_parameter_priors"   << std::endl;
    }
	...
	std::cout << "minLength            : ";
	if (TRACKER::minLength<(DEFAULT_PTT_WITH_PARAMETER_PRIORS_MINLENGTH*1.01))	std::cout 			<< "0" 							<< std::endl;
	else 								std::cout << TRACKER::minLength 		<< " " << TRACKER::img_FOD->getUnit() 	<< std::endl;

	std::cout << "maxLength            : ";
	if (TRACKER::maxLength>(DEFAULT_PTT_WITH_PARAMETER_PRIORS_MAXLENGTH*0.99))	std::cout 			<< "infinite " << img_FOD->getUnit() 	<< std::endl;

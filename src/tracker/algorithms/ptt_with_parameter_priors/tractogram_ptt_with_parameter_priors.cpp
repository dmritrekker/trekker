...
Tractogram_PTT_with_parameter_priors::Tractogram_PTT_with_parameter_priors() {
	...
	if (TRACKER::propMaxEstTrials==NOTSET)
		prop_postEstItCount 			= DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROP_POSTESTITCOUNT_INITIAL_CONDITION;
	else
	...
	if (TRACKER::initMaxEstTrials==NOTSET)
		init_postEstItCount 			= DEFAULT_PTT_WITH_PARAMETER_PRIORS_INIT_POSTESTITCOUNT_INITIAL_CONDITION;
	else
...
void Tractogram_PTT_with_parameter_priors::printSummary() {
...
void Tractogram_PTT_with_parameter_priors::update(TrackingThread *thread) {

	baseUpdate(thread);

	Streamline_PTT_with_parameter_priors *streamline 			 = ((Streamline_PTT_with_parameter_priors*)thread->streamline);
	...
	if (TRACKER::propMaxEstTrials==NOTSET) {
		if (streamline->sampling_init_fail==0) {
			...
			if (prop_pooler % prop_numberOfRecentStreamlines == 0) {
				...
				if (prop_postEstItCount < DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROP_MINPOSTESTIT)  prop_postEstItCount = DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROP_MINPOSTESTIT;
				if (prop_postEstItCount > DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROP_MAXPOSTESTIT)  prop_postEstItCount = DEFAULT_PTT_WITH_PARAMETER_PRIORS_PROP_MAXPOSTESTIT;
				prop_numberOfRecentFails = 0;
			}
	...
	if (TRACKER::initMaxEstTrials==NOTSET) {
		...
		if (total_generated % init_numberOfRecentStreamlines == 0) {
			...
			if (init_postEstItCount < DEFAULT_PTT_WITH_PARAMETER_PRIORS_INIT_MINPOSTESTIT)  init_postEstItCount = DEFAULT_PTT_WITH_PARAMETER_PRIORS_INIT_MINPOSTESTIT;
			if (init_postEstItCount > DEFAULT_PTT_WITH_PARAMETER_PRIORS_INIT_MAXPOSTESTIT ) init_postEstItCount = DEFAULT_PTT_WITH_PARAMETER_PRIORS_INIT_MAXPOSTESTIT;
			init_numberOfRecentFails = 0;
		}
	}
...
void Tractogram_PTT_with_parameter_priors::writeOutput() {
	...
	if (tangentWriteMode==WRITE_ON) {
		...
		for (size_t i=0; i<streamlineCount; i++)
			if (streamlines[i]->status == STREAMLINE_GOOD) {
				Streamline_PTT_with_parameter_priors *streamline = ((Streamline_PTT_with_parameter_priors *)streamlines[i]);
...
void Tractogram_PTT_with_parameter_priors::writeMetadataOutput() {

	FILE *out = writeBaseMetadataOutput();

	fprintf(out,",\n\"TRACKER OPTIONS\": {");
    fprintf(out,"\n\"algorithm\": \"probabilistic parallel transport tracker (ptt_with_parameter_priors) \" ");
	...
	if (TRACKER::maxLength==DEFAULT_PTT_WITH_PARAMETER_PRIORS_MAXLENGTH)
		fprintf(out,",\n\"maxLength\":\"infinite\"");
	else
